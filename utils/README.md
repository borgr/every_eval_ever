...
| `wild` | WILD-raw item-level dataset (HF: `kensho/WILD-raw`) | Converts item-level eval responses (65 models × 27 benchmarks, run with Inspect AI) into `data/wild/`; aggregate accuracy per model×benchmark (+ per-subtask), with optional per-item `_samples.jsonl` instance sidecars (`--include-instances`). |
