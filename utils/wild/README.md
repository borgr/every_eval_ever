...
```bash
uv run python -m utils.wild.adapter --output-dir /tmp/eee-wild-inst \
...
```bash
uv run python -m utils.wild.adapter --output-dir data/wild            # aggregates
uv run python -m utils.wild.adapter --output-dir data/wild --include-instances
```

Local parquet instead of fetching:

```bash
uv run python -m utils.wild.adapter --parquet data-000*.parquet --output-dir /tmp/eee-wild
```
